-- PROD PLAN · Paso 3/4 · migration name: prod_03_editorial_permissions
-- Contenido: supabase/04-editorial-permissions.sql VERBATIM (blob a9af6d4)
-- (mismas sentencias que supabase/migrations/20260921000200_editorial_permissions.sql).
-- Efecto en prod: crea editorial_members (FK auth.users on delete cascade, check role),
-- moderation_events (FKs a auth.users/submissions/verified_contributors, check action de 3 valores),
-- índices editorial_members_active_role_idx, moderation_events_actor_created_idx,
-- moderation_events_submission_idx; RLS activado (sin policies) y comentarios de tabla.
-- ─────────────────────────────────────────────────────────────────────────────
